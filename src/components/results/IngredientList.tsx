// components/results/IngredientList.tsx
"use client"
...
// Common harmful additives to detect (without API lookup)
...
export default function IngredientList({ ingredients, harmfulIngredients }: Props) {
  ...
  const parsedIngredients = useMemo(() => {
    ...
    const items = ingredients
      .replace(/\([\d\w\s,]*\)/g, '') // Remove parenthetical notes like (1.2%)
      .split(/[,;•]/)
    ...
    return items.map(name => {
      // Check against known harmful additives
      let risk = null
      for (const [additive, info] of Object.entries(HARMFUL_ADDITIVES)) {
        if (name.includes(additive)) {
          risk = info
          break
        }
      }
      
      // Check against analysis harmful ingredients
      if (!risk) {
    ...
    <div>
      <div className="flex flex-wrap gap-1.5">
        {displayItems.map((item, idx) => (
          ...
          >
            {item.risk && (
              <span className="mr-1">
                {item.risk.severity === 'critical' || item.risk.severity === 'high' ? '⚠️' : '⚠'}
              </span>
            )}
      ...
      {parsedIngredients.length > 8 && (
        <button
          ...
          className="mt-2 text-[11px] font-bold text-emerald-400 hover:text-emerald-300 transition-colors"
        >
      ...
      {harmfulCount > 0 && (
        <div className="mt-2 pt-2 border-t border-[#2a3545]">
