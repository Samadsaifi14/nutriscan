...
import {
  ...
  Droplets, ShoppingBag, Clock, Activity, Info, Zap
} from 'lucide-react'
...
export interface AnalysisData {
  ...
  ingredient_warnings?: Array<{ ingredient: string; concern: string; severity: 'high' | 'medium' | 'low' }>
...
function getSeverityStyles(severity: string) {
  switch (severity) {
    case 'high':   return { badge: 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 border border-red-200 dark:border-red-800', card: 'border-red-200 dark:border-red-900/50 bg-red-50/40 dark:bg-red-900/10', icon: 'text-red-500' }
    case 'medium': return { badge: 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-800', card: 'border-amber-200 dark:border-amber-900/50 bg-amber-50/40 dark:bg-amber-900/10', icon: 'text-amber-500' }
    default:       return { badge: 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400 border border-yellow-200 dark:border-yellow-800', card: 'border-yellow-200 dark:border-yellow-900/50 bg-yellow-50/40 dark:bg-yellow-900/10', icon: 'text-yellow-500' }
  }
}
    ...
    <div className="flex flex-col items-center gap-3">
      <div className="relative w-36 h-36">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-3xl font-black tabular-nums" style={{ color }}>{score.toFixed(1)}</span>
          <span className="text-xs text-gray-400 dark:text-gray-500 font-medium">/ 10</span>
        </div>
      </div>
      <span className={`px-4 py-1.5 rounded-full text-sm font-bold bg-gradient-to-r text-white shadow-sm
        ${ratingGradient[rating as keyof typeof ratingGradient] || ratingGradient.moderate}`}>
        {label}
      </span>
    </div>
  )
...
// ── Harmful Ingredient Accordion ─────────────────────────────────────────────
function HarmfulIngredientCard({ item }: { item: HarmfulIngredient }) {
  ...
  return (
    <div className={`rounded-xl border overflow-hidden ${s.card}`}>
      <button className="w-full flex items-start gap-3 p-3 text-left" onClick={() => setOpen(x => !x)}>
        ...
        <div className="flex-1 min-w-0">
          ...
          {item.also_known_as && item.also_known_as.length > 0 && (
            <p className="text-xs text-gray-400 mt-0.5">Also: {item.also_known_as.join(', ')}</p>
          )}
      ...
      {open && (
        <div className="px-3 pb-3 space-y-2.5 border-t border-white/20 dark:border-black/20 pt-2">
          <div className="grid grid-cols-2 gap-2">
            {item.amount_in_this_product && (
              <div className="bg-white/60 dark:bg-black/20 rounded-lg p-2.5">
                <p className="text-[10px] font-medium text-gray-400 uppercase tracking-wide mb-0.5">In this product</p>
                <p className="text-xs font-semibold text-gray-800 dark:text-gray-200">{item.amount_in_this_product}</p>
              </div>
            )}
            {item.global_safe_limit && (
              <div className="bg-white/60 dark:bg-black/20 rounded-lg p-2.5">
                <p className="text-[10px] font-medium text-gray-400 uppercase tracking-wide mb-0.5">Daily safe limit</p>
            ...
            {item.percentage_of_daily_limit && (
              <div className="bg-white/60 dark:bg-black/20 rounded-lg p-2.5">
                <p className="text-[10px] font-medium text-gray-400 uppercase tracking-wide mb-0.5">% of daily limit</p>
            ...
            {item.personalized_safe_limit && (
              <div className="bg-white/60 dark:bg-black/20 rounded-lg p-2.5">
                <p className="text-[10px] font-medium text-gray-400 uppercase tracking-wide mb-0.5">Your safe limit</p>
...
// ── Main Component ────────────────────────────────────────────────────────────
export default function AnalysisCard({ analysis, productName }: Props) {
  ...
  // Derive long-term risks from ingredient concerns if not explicitly provided
  const longTermRisks = analysis?.long_term_risks?.length
    ? analysis.long_term_risks
    : harmful.filter(h => h.severity === 'high').map(h => h.concern)

  const alternatives = analysis?.healthier_alternatives || []
  const positives    = analysis?.positives || []

  const suitability = [
    { label: 'Diabetic',      value: analysis?.diabetic_suitability,  Icon: Droplets  },
    { label: 'High BP',       value: analysis?.bp_suitability,        Icon: Heart     },
    { label: 'Children',      value: analysis?.child_suitability,     Icon: Baby      },
    { label: 'Pregnancy',     value: analysis?.pregnancy_suitability, Icon: Activity  },
  ]
    ...
    <div className="space-y-3 animate-fade-in-up">
      ...
      {suitability.some(s => s.value && s.value !== 'unknown') && (
        <div className="rounded-2xl p-4 bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 shadow-sm">
          <p className="text-[10px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-3">Health Suitability</p>
          <div className="grid grid-cols-2 gap-2">
            {suitability.map(({ label, value, Icon }) => {
      ...
      {/* ── HARMFUL INGREDIENTS ──────────────────────────────────── */}
      {harmful.length > 0 && (
        <div className="rounded-2xl p-4 bg-white dark:bg-gray-900 border border-red-100 dark:border-red-900/30 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            <p className="text-sm font-bold text-red-600 dark:text-red-400 flex items-center gap-2">
              <AlertTriangle className="w-4 h-4" />
              {harmful.length} Harmful Ingredient{harmful.length !== 1 ? 's' : ''} Detected
            </p>
            {highRisk.length > 0 && (
          ...
          <div className="space-y-2">
            {harmful.map((item, i) => <HarmfulIngredientCard key={i} item={item} />)}
          </div>
        </div>
      )}
      ...
      {/* ── INGREDIENT WARNINGS (fallback when no harmful_ingredients) ── */}
      {(analysis?.ingredient_warnings || []).length > 0 && harmful.length === 0 && (
        <div className="rounded-2xl p-4 bg-white dark:bg-gray-900 border border-amber-100 dark:border-amber-900/30 shadow-sm">
          ...
          <ul className="space-y-2.5">
            {analysis!.ingredient_warnings!.map((w, i) => {
              ...
              return (
                <li key={i} className="flex items-start gap-2.5">
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium flex-shrink-0 mt-0.5 capitalize ${s.badge}`}>
                    {w.severity}
                  </span>
                  <div>
                    <p className="text-sm font-semibold text-gray-800 dark:text-gray-200">{w.ingredient}</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed">{w.concern}</p>
                  </div>
                </li>
              )
            })}
          ...
          <div className="grid grid-cols-2 gap-2.5">
            {analysis.safe_consumption.amount && (
              <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-3">
                <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wide mb-1">Safe Amount</p>
            ...
            {analysis.safe_consumption.frequency && (
              <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-3">
                <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wide mb-1">Frequency</p>
      ...
      {analysis?.fssai_compliance && (
        <div className={`rounded-xl px-4 py-3 flex items-center gap-3 text-sm font-medium
          ...
          FSSAI:{' '}
          {analysis.fssai_compliance === 'compliant' ? 'No compliance concerns detected'
