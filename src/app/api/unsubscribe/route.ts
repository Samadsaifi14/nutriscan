...
export async function GET(req: NextRequest) {
  ...
  if (!userId || !exp || !sig) {
    return new NextResponse(buildHTML('Invalid or expired unsubscribe link.', false), {
      headers: { 'Content-Type': 'text/html' },
    })
  }

  if (!verifyUnsubscribeToken(userId, type, exp, sig)) {
    return new NextResponse(buildHTML('Invalid or expired unsubscribe link.', false), {
      headers: { 'Content-Type': 'text/html' },
    })
  }
  ...
  try {
    ...
    const message =
      type === 'weekly'
        ...
        : 'You have been unsubscribed from all NutriScan emails.'
...
<head>
  ...
  <title>NutriScan — Unsubscribe</title>
</head>
<body style="margin:0;padding:0;background:#F2EDE4;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;min-height:100vh;display:flex;align-items:center;justify-content:center;">
  <div style="max-width:400px;margin:0 auto;padding:32px 16px;text-align:center;">
    ...
    <div style="background:white;border-radius:20px;padding:32px;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
      ...
      <a href="${appUrl}/dashboard"
        style="display:inline-block;padding:12px 28px;background:#C4714A;color:white;text-decoration:none;border-radius:12px;font-size:14px;font-weight:700;">
        Go to NutriScan →
      </a>
    </div>
